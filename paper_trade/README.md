...
- [x] jialong 确认 paper-trade 路径 (spec v3 BB-only)
- [x] 改 `config.json`: `enabled=true`, `phase="live_phase1"`, `started_at="2026-04-17"`
- [x] 配 launchd daily `~/Library/LaunchAgents/dev.quantdojo.paper-trade-daily.plist` (Mon-Fri 09:30 本地 = SH 21:30)
- [x] 配 launchd monthly `~/Library/LaunchAgents/dev.quantdojo.paper-trade-monthly.plist` (每月 1 号 10:00)
- [x] macOS 本地通知 (osascript) 当 kill action != OK 时自动弹通知
- [ ] **TODO**: 刷新 tushare token (当前过期, `.env` 里的 token 返回 "您的token不对"); 刷了以后 backfill 2026-04-18 ~ 当日
- [ ] (可选) 绑定 alerts.log → Slack/邮件 webhook, 目前仅本地通知 + 日志

### launchd 管理命令

```bash
# 查看是否加载
launchctl list | grep quantdojo

# 查看下次触发时间 + 上次 exit code
launchctl print gui/$(id -u)/dev.quantdojo.paper-trade-daily | head -30
...
`launchd` 在机器睡眠时不会触发 scheduled 任务. 如果错过一天, `paper_trade_daily.py`
**不会自动补跑**; 手动 backfill:

```bash
python scripts/paper_trade_daily.py --date 2026-04-XX
```

跑当天的 state.json / orders csv / daily report 会落盘, trader SQLite ledger 从
上一次的状态续下去 (天然幂等).
