"""
v12 候选研究：V10 + 半仓择时（regime_scale=0.5）

核心改动（相对 v11）：
  - 看空时不清仓，而是减仓至 50%（15 支股票）
  - 保留上行弹性，同时降低市场 beta 暴露
  - 其他参数与 v11 完全一致（三指标多数投票择时）

对比体系：
  v10 : 6 因子，无择时（满仓）
  v11 : 6 因子，有择时，看空清仓（0 仓）
  v12 : 6 因子，有择时，看空减仓（50% 仓）

运行：python scripts/v12_partial_regime_eval.py
"""
import sys
import time
import warnings
...
def build_regime_mask(hs300_full):
    print("\n[2/5] 构建择时信号（多数投票）...")
    ...
    print(f"  IS 看多: {is_mask.mean():.0%}  看空: {1-is_mask.mean():.0%}  "
          f"（看空时持 {int(N_STOCKS * REGIME_SCALE)} 支股，非清仓）")
    return mask
...
def run_is_oos(price, neutral, tradable, weights, signs, regime_mask, hs300_full):
    print("\n[4/5] IS/OOS 回测对比（v10 vs v11 vs v12）...")
    hs300_ret = hs300_full["close"].pct_change().dropna()
    ns = apply_signs(neutral, signs)

    ret_v10 = run_backtest(price, ns, weights, n_stocks=N_STOCKS, cost=COST,
                           mask=tradable, regime_mask=None, regime_scale=0.0, lag1=True)
    ret_v11 = run_backtest(price, ns, weights, n_stocks=N_STOCKS, cost=COST,
                           mask=tradable, regime_mask=regime_mask, regime_scale=0.0, lag1=True)
    ret_v12 = run_backtest(price, ns, weights, n_stocks=N_STOCKS, cost=COST,
                           mask=tradable, regime_mask=regime_mask, regime_scale=REGIME_SCALE, lag1=True)

    header = (f"  {'策略':<16} {'年化':>8} {'波动':>8} {'夏普':>7} "
              f"{'最大回撤':>10} {'卡玛':>7} {'超额':>8}")
    sep = "  " + "-" * 68
    ...
    for period, s, e in [("IS (2015-2024)", IS_START, IS_END),
        ...
        print(header)
        print(sep)
        for name, ret in [("v10（无择时）", ret_v10),
                           ("v11（清仓）",   ret_v11),
                           ("v12（半仓）",   ret_v12)]:
            seg = ret.loc[s:e]
            if len(seg) < 5:
            ...
            print(f"  {name:<16} {m.get('ann',0):>8.2%} {m.get('vol',0):>8.2%} "
    ...
    print(f"  {'段':12} {'策略':<16} {'累计':>8} {'夏普':>7} {'最大回撤':>10}")
    print("  " + "-" * 57)
    for seg_label, s, e in stress:
        for name, ret in [("v10（无择时）", ret_v10),
            ...
            print(f"  {seg_label:<12} {name:<16} {cum:>8.2%} {sr:>7.3f} {mdd:>10.2%}")
        bull = regime_mask.loc[s:e].mean()
        print(f"  {'':12} {'↳ 择时看多':16} {bull:>8.0%}")
        print()

    return ret_v10, ret_v11, ret_v12


def run_wf(price, pb, industry_df, regime_mask):
    print(f"\n[5/5] Walk-Forward（v12，半仓择时）...")
    ...
    def wf_fn(price_slice, _fdata, train_start, train_end, test_start, test_end):
        ...
        signs   = w_res["signs"]
        ns      = {n: f * signs.get(n, 1) for n, f in neutral.items()}
        local_regime = regime_mask.reindex(price_slice.index).ffill().fillna(True)

        wf_ret = run_backtest(
            price_slice, ns, weights,
            n_stocks=N_STOCKS, cost=COST,
            mask=None, regime_mask=local_regime,
            regime_scale=REGIME_SCALE, lag1=True,
        )
...
def write_report(ret_v10, ret_v11, ret_v12, weights, signs, wf_df, regime_mask):
    out = Path(__file__).parent.parent / "journal" / f"v12_partial_regime_eval_{date.today().strftime('%Y%m%d')}.md"
    ...
    for label, s, e in [("2022熊市","2022-01-01","2022-10-31"),
        ...
        c12 = (1 + ret_v12.loc[s:e].fillna(0)).prod() - 1
        stress_rows.append(f"| {label} | {c10:+.2%} | {c11:+.2%} | {c12:+.2%} |")
    ...
    if wf_df is not None:
        ...
        wf_lines = [
            ...
            f"| 指标 | v10 | v11 | v12 |",
            f"|---|---|---|---|",
            f"| 夏普中位数 | 0.5829 | 0.4450 | {valid['sharpe'].median():.4f} |",
            f"| 夏普均值   | — | 0.7653 | {valid['sharpe'].mean():.4f} |",
            f"| 回撤均值   | — | -10.05% | {valid['max_drawdown'].mean():.2%} |",
            f"| 胜率       | 65% | — | {(valid['total_return']>0).mean():.0%} |",
        ]

    lines = [
        f"# v12 半仓择时评估报告 — {date.today()}",
        "",
        "## 核心改动",
        "",
        ...
        f"- v11（清仓）  : {fmt(ret_v11, IS_START, IS_END)}",
        f"- v12（半仓）  : {fmt(ret_v12, IS_START, IS_END)}",
        "",
        "## OOS 表现（2025）",
        f"- v10（无择时）: {fmt(ret_v10, OOS_START, OOS_END)}",
        f"- v11（清仓）  : {fmt(ret_v11, OOS_START, OOS_END)}",
        f"- v12（半仓）  : {fmt(ret_v12, OOS_START, OOS_END)}",
        "",
        "## 急跌行情对比",
        "",
        "| 段 | v10 | v11（清仓） | v12（半仓） |",
        "|---|---|---|---|",
    ] + stress_rows + wf_lines + ["", "## 结论", "", "（待填写）"]
