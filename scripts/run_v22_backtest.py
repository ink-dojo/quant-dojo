"""
运行 multi_factor_v22 单次 in-sample 回测，写入 live/runs/。

v22 = v16 正交性剪枝：剔除 win_rate_60d / mom_6m_skip1m / high_52w / amihud_illiq
（Gram-Schmidt 残差 IC 95%CI 跨零），保留 5 个独立因子。

回测期：2022-01-01 ~ 2025-12-31（与 v16 同 IS 窗口，便于直接对比）。

运行：python scripts/run_v22_backtest.py
"""
...
START = "2022-01-01"
END = "2025-12-31"
WARMUP_START = "2019-01-01"  # v16 策略需要 lookback
N_STOCKS = 30
...
def main():
    ...
    print("[2/4] 构建 v22 策略…")
    entry = get_strategy("multi_factor_v22")
    params = {"n_stocks": N_STOCKS}
    strategy = entry.factory(params)

    print("[3/4] 运行回测（可能需要数分钟）…")
    ...
    run_id = generate_run_id("multi_factor_v22", START, END, params)
