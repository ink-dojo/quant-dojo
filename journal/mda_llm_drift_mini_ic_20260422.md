...
- LLM: claude -p (model per script config),  prompt: 5 维度 drift + 脱敏 + 禁止训练知识
...
## 追加分析 (2026-04-22 晚) — Industry-Neutral 重跑

**背景**: 初版 IC 未做行业中性化. 疑虑: 新能源 MD&A 天然比银行 MD&A 自由发挥, 分数差异可能来自行业效应而非 within-industry alpha.

**方法**: 申万一级行业 (31 类, 覆盖全部 429 样本) 做 within-industry demean, 重跑 pooled IC + 1000 次 bootstrap CI.

| 维度 | raw IC | ind-neu IC | 变化 | Bootstrap %dir (ind-neu) |
|---|---:|---:|---:|---:|
| specificity | -0.0260 | -0.0017 | **+0.024 (方向消失)** | 52.5% |
| hedging | -0.0513 | -0.0525 | -0.001 | 87.7% |
| tone | -0.0514 | -0.0287 | +0.023 (弱化) | 74.3% |
| **forward** | **-0.0677** | **-0.0808** | **-0.013 (强化)** | **95.0%** (刚好单边阈值) |
| transparency | +0.0880 | +0.0747 | -0.013 (略弱化) | 92.7% |

**解读:**
- `specificity` 的原始信号 **完全是行业效应** (化工 MD&A 数字多 / 银行 MD&A 数字少), industry-neutral 后方向消失 — 去伪
- `forward` industry-neutral 后 **%dir 从 90.4% → 95.0%**, 刚好达单边 95% 显著阈值. 配合 order group diag 差 0.004 (最干净), 这是全部实验里**最接近"真 signal"的证据**
- `transparency` 略微弱化但仍 92.7%, 部分来自"新能源这类行业本身更坦率"的 between-industry 效应, 但 within-industry 仍显示 drift 高 → 未来 return 略好
- 两侧 95% CI 仍跨 0, 严格意义仍**不显著**

**结论修正**: 原 KILL 判读过于激进. 这批数据不是 "没信号", 而是 "时序 3 月太短, bootstrap n 够但 regime 单一". forward drift 这一维在 2024 cross-section 上**有真实但弱的信号**.

**真正不扩 full sample 的理由 (修正版)**:
1. 样本时序覆盖单一季度 (2025-02~04), 扩到 2600 对仍然是同样 3 个月 × 多年, 需要至少 2 个独立 regime 才能 replicate
2. 应该先做**风险段 drift** (只切 MD&A 的"风险提示"段, 纯度可能远高于全文) — 若风险段 forward_drift industry-neutral IC 能过 0.08 且 CI_low > 0, 就真有信号了
3. Correlation 和 RIAD/MFD/BGFD 对齐需要重新生成日度因子面板 (~2-3h 工程), 不阻塞主决策

**保留的后续路径** (不阻塞 Tier 3 换数据源的主方向):
- [ ] 风险段 drift 实验 (2-3h)
- [ ] Correlation with 已有因子 (2-3h)
- [ ] 若两者都有正向 → 再考虑扩 full sample
