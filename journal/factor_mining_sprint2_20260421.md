...
- Δ 年化 **+3.65%**
- Δ 夏普 **+0.195**
- Δ MDD **改善 +3.39pp** (反转因子天然降 MDD)
- Δ 超额 +2.84%

ind_reversal_3m 在 ICIR 加权下获 **9.9% 权重**, ICIR +0.313 (被 neg_crowding 的 +1.073 压制)。

> 注: 本次 v18 OOS 运行时外置盘访问被系统撤回, bp 因子以常数 1.0 占位 (ICIR 0)。
> 故 v17 baseline 比 sprint 1 里的 Sharpe 1.455 低了 0.17, 是 bp 缺失造成。
> 相对 delta (v18 - v17 = +0.195 Sharpe) 在公平对比下依然成立。
...
1. **正式化 v18**: 把 ind_reversal_3m 纳入主因子池, 更新 `scripts/v17_*` → `v18_*`
2. **补做 avoid filter**: 封装 `utils/filters/lockup_pressure_filter.py`, 在 portfolio 层面统一用
3. **重跑 v18 带真 bp**: 恢复外置盘访问后用真 pb 数据重跑验证
