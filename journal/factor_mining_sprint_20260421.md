...
| F3 | attention_60d (特定对象调研, warmup 修复后) | stk_surv | -0.040 | **-0.50** | **-4.42** | ✅ 反向 alpha (取负) |
| F5 | **composite_crowding (size-neutral v2)** | 合成 | **~0.10** | **0.83** | **7.13** | ✅ **合格, 多空年化 17.58% 夏普 1.56** |
| F6 | inst_ratio (ann_date 对齐修复后) | top10_floatholders | 0.0089 | **0.30** | **2.36** | ⚠️ 边缘合格 (从 0.04 → 0.30, 季度信号非 dead) |
| — | (turnover_20d 作 F5 子维度) | daily_basic | — | 0.50 | 4.36 | ✅ F5 主导驱动 |

> **Code review 修复**（2026-04-21 傍晚）：
> - F3 warmup 修复：reindex 到 WARMUP_START 2021-08-01 构造滚动窗口，避免 2022 Q1 出现 0 事件伪值
> - F5 v2 去掉 `fillna(0)` 结构性偏差，新增 size-neutral 版本 (`composite_crowding_sn.parquet`)，ICIR 从 0.73 升到 0.83 — 说明 size 是噪声不是驱动力
> - F6 ann_date 修复：从 `end_date + 60d 固定 lag` 改为 `ann_date 次日交易日生效`，ICIR 从 0.04 拉到 0.30，从 fail 变边缘合格
...
### **严格 OOS：2022-01~2024-12 学权重 → 2025 测试（size-neutral crowding 重跑）**
| 策略 | 阶段 | 年化 | 夏普 | MDD | 超额 |
|---|---|---:|---:|---:|---:|
...
**OOS 差异**：年化 +1.98%, 夏普 +0.061, MDD 改善 0.33pp, 超额 +1.68%

> ⚠️ 注：修 F5 的结构性 fillna(0) 和 size 偏差后，v17 相对 v9 的 OOS edge 从 +0.186 缩到 +0.061 夏普。
> 这印证了之前一部分"超额"其实是未 SN 的 size 效应在吃 beta。SN 版本 ICIR 更高但 edge 更薄是**真更干净**的信号。

### 结论
**✅ v17 严格 OOS 仍胜出**，但 edge 比修复前减半。-composite_crowding (size-neutral) 带来**真实但边际的**增量 alpha。
训练期 neg_crowding ICIR 1.11 (六因子最高), 获 20.9% 权重, 在 2025 OOS 贡献 +1.68% 超额。

推荐：**谨慎采纳** — 正式把 crowding 纳入因子池，但不要高估其贡献；继续寻找更独立的 alpha 源。
